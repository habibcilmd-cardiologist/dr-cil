---
title: "Pericardiocentesis - Pericardial Fluid Drainage"
description: "Expert pericardiocentesis in Istanbul. Assoc. Prof. Dr. Habib Çil performs pericardial fluid drainage. Pericardial effusion treatment. Specialist cardiology at Avrasya Hospital."
date: 2024-01-15
lastmod: 2024-12-01
draft: false
service_type: "MedicalProcedure"
medical_specialty: "Cardiology"
procedure_type: "TherapeuticProcedure"
body_location: "Heart"
translationKey: "pericardiocentesis"
showBreadcrumbs: true
showTableOfContents: true
showReadingTime: true
faq:
    - question: "Is pericardiocentesis painful?"
      answer: "It is performed under local anesthesia and usually no pain is felt."
    - question: "Does fluid accumulate again after pericardiocentesis?"
      answer: "It depends on the underlying cause. It may recur in some patients."
    - question: "How long does pericardiocentesis take?"
      answer: "The procedure usually takes 30-60 minutes."
    - question: "When will I be discharged after pericardiocentesis?"
      answer: "Usually discharge is possible after 1-2 days of observation."
    - question: "What are the risks of pericardiocentesis?"
      answer: "Serious complications are rare. Heart perforation risk is low."
---

Pericardiocentesis is draining fluid accumulated between the pericardium (heart sac) and the heart with a needle. Assoc. Prof. Dr. Habib Çil has extensive experience in pericardiocentesis procedures.

## What is Pericardiocentesis?

Pericardiocentesis is draining fluid accumulated in the pericardial space with a needle and catheter. It is performed for both diagnostic and therapeutic purposes.

**Purposes of Pericardiocentesis:**

-   Diagnostic (fluid analysis)
-   Therapeutic (fluid drainage)
-   Emergency (tamponade)

## Causes of Pericardial Effusion

### Infectious

-   Viral pericarditis
-   Bacterial pericarditis
-   Tuberculous pericarditis

### Non-infectious

-   Malignancy
-   Uremia
-   Autoimmune diseases
-   Post-cardiac surgery
-   Trauma
-   Idiopathic

## Pericardiocentesis Indications

### Emergency Indications

-   Cardiac tamponade
-   Hemodynamic instability
-   Shock

### Elective Indications

-   Large effusion
-   Symptomatic effusion
-   Diagnostic purpose
-   Recurrent effusion

## How is Pericardiocentesis Performed?

### Preparation

-   Evaluation with echocardiography
-   Fluid localization
-   Coagulation control

### Procedure Techniques

**Subxiphoid Approach:**

-   Most common technique
-   Entry below xiphoid
-   Echocardiography-guided

**Apical Approach:**

-   Entry from heart apex
-   In selected cases

### Procedure Steps

1. **Local anesthesia**: To entry site
2. **Needle entry**: Echocardiography-guided
3. **Fluid aspiration**: With syringe
4. **Catheter placement**: If needed
5. **Fluid drainage**: Complete drainage
6. **Catheter removal**: End of procedure

### Procedure Duration

Total duration: 30-60 minutes

## Pericardiocentesis Results

### Success Rates

-   Technical success: >95%
-   Symptom improvement: >90%
-   Complication rate: <5%

### Fluid Analysis

-   Cytology
-   Biochemistry
-   Microbiology
-   Culture

## Complications

-   Heart perforation (rare)
-   Arrhythmia
-   Pneumothorax
-   Infection
-   Bleeding

## After Pericardiocentesis

### Follow-up

-   Echocardiography control
-   Fluid recurrence monitoring
-   Treatment of underlying cause

### Recurrent Effusion

-   Pericardial window
-   Pericardiectomy
-   Sclerotherapy

## Frequently Asked Questions

{{< faq-list >}}

## Appointment and Contact

If you would like to schedule an appointment for pericardiocentesis evaluation:

{{< button href="https://wa.me/902126655050" target="_blank" >}}
Book Appointment via WhatsApp
{{< /button >}}

📍 **Avrasya Hospital** - Beştelsiz Mah. 101. Sokak No:107, Zeytinburnu, Istanbul

📞 **Phone:** +90 212 665 50 50 (Ext: 4012)

---

_This content has been prepared by Assoc. Prof. Dr. Habib Çil for general informational purposes. Please consult a cardiology specialist for definitive diagnosis and treatment._
