---
title: "LVAD - Sol Ventrikül Destek Cihazı"
description: "İstanbul'da LVAD değerlendirmesi. Doç. Dr. Habib Çil ile sol ventrikül destek cihazı takibi. Mekanik kalp desteği. Avrasya Hospital'da uzman kardiyoloji."
date: 2024-01-15
lastmod: 2024-12-01
draft: false
service_type: "MedicalProcedure"
medical_specialty: "Cardiology"
procedure_type: "TherapeuticProcedure"
body_location: "Heart"
translationKey: "lvad"
showBreadcrumbs: true
showTableOfContents: true
showReadingTime: true
---

LVAD (Sol Ventrikül Destek Cihazı), ileri kalp yetmezliğinde kalbin pompalama fonksiyonunu destekleyen mekanik cihazdır. Doç. Dr. Habib Çil, LVAD hastalarının takibinde geniş deneyime sahiptir.

## LVAD Nedir?

LVAD, sol ventrikülden kanı alarak aortaya pompalayan mekanik cihazdır. Kalbin pompalama fonksiyonunu destekler veya tamamen üstlenir.

**LVAD Bileşenleri:**

-   Pompa (kalp içine yerleştirilir)
-   Driveline (vücut dışına çıkan kablo)
-   Kontrolör (pompa kontrolü)
-   Bataryalar (güç kaynağı)

## LVAD Endikasyonları

### Köprü Tedavisi

-   Kalp nakline köprü
-   İyileşmeye köprü
-   Karara köprü

### Hedef Tedavi

-   Kalp nakline uygun olmayan hastalar
-   Kalıcı mekanik destek
-   Yaşam kalitesi iyileştirme

### Hasta Seçimi

-   Son dönem kalp yetmezliği
-   Maksimal medikal tedaviye rağmen semptomlar
-   Ejeksiyon fraksiyonu <%25
-   İnotropik bağımlılık

## LVAD Kontrendikasyonları

### Mutlak Kontrendikasyonlar

-   Sağ kalp yetmezliği (ciddi)
-   İrreversible çoklu organ yetmezliği
-   Aktif enfeksiyon
-   Koagülopati

### Göreceli Kontrendikasyonlar

-   İleri yaş
-   Böbrek yetmezliği
-   Karaciğer yetmezliği
-   Psikososyal sorunlar

## LVAD Ameliyatı

### Ameliyat Öncesi

-   Kapsamlı değerlendirme
-   Sağ kalp fonksiyonu
-   Organ fonksiyonları
-   Psikososyal değerlendirme

### Ameliyat Tekniği

1. **Sternotomi**: Göğüs açılması
2. **Kalp-akciğer makinesi**: Bağlantı
3. **Pompa yerleştirme**: Sol ventrikül apeksine
4. **Çıkış grefti**: Aortaya bağlantı
5. **Driveline**: Karın duvarından çıkış

### Ameliyat Süresi

Toplam süre: 4-6 saat

## LVAD Sonrası Yaşam

### Günlük Bakım

-   Driveline bakımı
-   Batarya değişimi
-   Kontrolör takibi
-   Enfeksiyon önleme

### Antikoagülasyon

-   Warfarin kullanımı
-   INR takibi (2.0-3.0)
-   Aspirin
-   Kanama riski

## LVAD Komplikasyonları

### Erken Komplikasyonlar

-   Kanama
-   Sağ kalp yetmezliği
-   Enfeksiyon
-   Aritmi

### Geç Komplikasyonlar

-   Driveline enfeksiyonu
-   Pompa trombozu
-   İnme
-   Gastrointestinal kanama

## LVAD Sonuçları

### Sağkalım

-   1 yıllık sağkalım: %80-85
-   2 yıllık sağkalım: %70-75
-   Yaşam kalitesi iyileşmesi

## Sık Sorulan Sorular

{{< faq question="LVAD ile normal yaşam mümkün mü?" >}}
Evet, birçok hasta günlük aktivitelerini sürdürebilir.
{{< /faq >}}

{{< faq question="LVAD bataryaları ne kadar dayanır?" >}}
Bataryalar genellikle 10-12 saat dayanır ve değiştirilebilir.
{{< /faq >}}

{{< faq question="LVAD ile yüzebilir miyim?" >}}
Hayır, cihaz suya dayanıklı değildir.
{{< /faq >}}

{{< faq question="LVAD kalıcı mı?" >}}
Köprü tedavisinde geçici, hedef tedavide kalıcıdır.
{{< /faq >}}

{{< faq question="LVAD sonrası MR çekilebilir mi?" >}}
Hayır, LVAD MR uyumlu değildir.
{{< /faq >}}

## Randevu ve İletişim

LVAD değerlendirmesi için randevu almak isterseniz:

{{< button href="https://wa.me/902126655050" target="_blank" >}}
WhatsApp ile Randevu Al
{{< /button >}}

📍 **Avrasya Hospital** - Beştelsiz Mah. 101. Sokak No:107, Zeytinburnu, İstanbul

📞 **Telefon:** +90 212 665 50 50 (Dahili: 4012)

---

_Bu içerik Doç. Dr. Habib Çil tarafından hazırlanmış olup, genel bilgilendirme amaçlıdır. Kesin tanı ve tedavi için mutlaka bir kardiyoloji uzmanına başvurunuz._
