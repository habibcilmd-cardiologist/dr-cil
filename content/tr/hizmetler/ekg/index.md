---
title: "EKG - 12 Derivasyonlu Elektrokardiyografi"
description: "İstanbul'da EKG çekimi. Kardiyoloji uzmanı Doç. Dr. Habib Çil ile kalp ritmi ve elektriksel aktivite değerlendirmesi. Avrasya Hospital."
date: 2024-01-15
lastmod: 2024-12-04
draft: false
service_type: "MedicalProcedure"
medical_specialty: "Cardiology"
procedure_type: "DiagnosticProcedure"
body_location: "Heart"
translationKey: "ekg"
showBreadcrumbs: true
showTableOfContents: true
showReadingTime: true
faq:
    - question: "EKG ağrılı mı?"
      answer: "Hayır, EKG tamamen ağrısız bir işlemdir. Sadece elektrotların cilde yapıştırılması gerekir."
    - question: "EKG için hazırlık gerekir mi?"
      answer: "Özel bir hazırlık gerekmez. Ancak göğüs bölgesinin açık olması gerektiğinden rahat giysiler tercih edilebilir."
    - question: "EKG sonucu ne zaman çıkar?"
      answer: "EKG sonucu hemen değerlendirilir ve yorumlanır."
    - question: "Normal EKG kalp hastalığı olmadığını gösterir mi?"
      answer: "Normal EKG önemli bir bulgudur ancak bazı kalp hastalıkları EKG'de görülmeyebilir. Ek testler gerekebilir."
    - question: "EKG ne sıklıkta çekilmeli?"
      answer: "Semptomlar, risk faktörleri ve mevcut hastalıklara göre doktor tarafından belirlenir."
---

Elektrokardiyografi (EKG), kalbin elektriksel aktivitesini kaydeden basit, ağrısız ve hızlı bir tanı yöntemidir. Kardiyolojide en sık kullanılan tetkiklerden biridir. Kardiyoloji uzmanı Doç. Dr. Habib Çil, EKG yorumlamasında geniş deneyime sahiptir.

## EKG Nedir?

EKG, kalbin her atımında ürettiği elektriksel sinyalleri cilt üzerine yerleştirilen elektrotlar aracılığıyla kaydeder. Bu kayıt, kalbin ritmi, hızı ve elektriksel iletimi hakkında değerli bilgiler sağlar.

**EKG'nin Değerlendirdiği Parametreler:**

-   Kalp hızı
-   Kalp ritmi (düzenli/düzensiz)
-   Elektriksel iletim
-   Kalp kasının durumu
-   Önceki kalp krizi izleri

## EKG Endikasyonları

### Semptom Değerlendirmesi

-   Göğüs ağrısı
-   Çarpıntı
-   Nefes darlığı
-   Baş dönmesi
-   Bayılma (senkop)
-   Yorgunluk

### Hastalık Takibi

-   Koroner arter hastalığı
-   Kalp yetmezliği
-   Aritmi takibi
-   Hipertansiyon
-   Kalp kapak hastalıkları

### Rutin Değerlendirme

-   Ameliyat öncesi değerlendirme
-   Check-up
-   Sporcu taraması
-   İlaç takibi

## EKG Nasıl Çekilir?

### Hazırlık

-   Özel hazırlık gerekmez
-   Üst giysilerin çıkarılması
-   Rahat pozisyonda yatma
-   Hareket etmeme

### İşlem Adımları

1. **Elektrot yerleştirme**: 10 elektrot (4 ekstremite, 6 göğüs)
2. **Kayıt**: Birkaç saniye sürer
3. **Elektrotların çıkarılması**: Jel temizlenir
4. **Sonuç**: Hemen değerlendirilir

### İşlem Süresi

EKG çekimi yaklaşık 5-10 dakika sürer.

## EKG Dalgaları ve Anlamları

### Normal EKG Bileşenleri

| Dalga/Segment | Anlam                                 |
| ------------- | ------------------------------------- |
| P dalgası     | Atriyal depolarizasyon                |
| PR aralığı    | AV iletim süresi                      |
| QRS kompleksi | Ventriküler depolarizasyon            |
| ST segmenti   | Ventriküler repolarizasyon başlangıcı |
| T dalgası     | Ventriküler repolarizasyon            |
| QT aralığı    | Toplam ventriküler aktivite           |

### Normal Değerler

-   Kalp hızı: 60-100/dk
-   PR aralığı: 120-200 ms
-   QRS süresi: <120 ms
-   QT aralığı: Hıza göre düzeltilmiş

## EKG'de Saptanan Anormallikler

### Ritim Bozuklukları

-   Sinüs taşikardisi/bradikardisi
-   Atriyal fibrilasyon
-   Atriyal flutter
-   Supraventriküler taşikardi
-   Ventriküler taşikardi
-   Prematür atımlar (PAC, PVC)

### İletim Bozuklukları

-   AV bloklar (1., 2., 3. derece)
-   Dal blokları (sağ/sol)
-   Fasikül blokları

### İskemi/İnfarktüs Bulguları

-   ST elevasyonu (akut MI)
-   ST depresyonu (iskemi)
-   T dalga inversiyonu
-   Q dalgaları (eski MI)

### Diğer Bulgular

-   Hipertrofi (sol/sağ ventrikül)
-   Elektrolit bozuklukları
-   Perikardit
-   Pulmoner emboli bulguları

## EKG Türleri

**Standart 12 Derivasyonlu EKG:**

-   En yaygın kullanılan
-   Kalbin farklı açılardan görüntülenmesi

**Holter EKG:**

-   24-48 saat sürekli kayıt
-   Aralıklı aritmilerin tespiti

**Olay Kaydedici:**

-   Semptom anında kayıt
-   Uzun süreli takip

## Sık Sorulan Sorular

{{< faq-list >}}

## Randevu ve İletişim

EKG çekimi ve değerlendirmesi için randevu almak isterseniz:

{{< button href="https://wa.me/902126655050" target="_blank" >}}
WhatsApp ile Randevu Al
{{< /button >}}

📍 **Avrasya Hospital** - Beştelsiz Mah. 101. Sokak No:107, Zeytinburnu, İstanbul

📞 **Telefon:** +90 212 665 50 50 (Dahili: 4012)

---

_Bu içerik Doç. Dr. Habib Çil tarafından hazırlanmış olup, genel bilgilendirme amaçlıdır. Kesin tanı ve tedavi için mutlaka bir kardiyoloji uzmanına başvurunuz._

## İlgili Tanısal Yöntemler

EKG ile bağlantılı diğer kardiyoloji testleri:

-   [Holter](/tr/hizmetler/holter/) - 24 saat ritim takibi
-   [Efor Testi](/tr/hizmetler/efor-testi/) - Egzersiz stres testi
-   [Ekokardiyografi](/tr/hizmetler/ekokardiyografi/) - Kalp ultrasonografisi
-   [Kardiyak BT](/tr/hizmetler/kardiyak-bt/) - Bilgisayarlı tomografi
-   [Miyokard Perfüzyon](/tr/hizmetler/miyokard-perfuzyon/) - Sintigrafi
