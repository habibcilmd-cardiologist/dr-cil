---
title: "Kardiyak BT - Koroner BT Anjiyografi (CCTA)"
description: "İstanbul'da kardiyak BT. Kardiyoloji uzmanı Doç. Dr. Habib Çil ile koroner BT anjiyografi. Non-invaziv kalp damar görüntüleme. Avrasya Hospital."
date: 2024-01-15
lastmod: 2024-12-04
draft: false
service_type: "MedicalProcedure"
medical_specialty: "Cardiology"
procedure_type: "DiagnosticProcedure"
body_location: "Heart"
translationKey: "cardiac-ct"
showBreadcrumbs: true
showTableOfContents: true
showReadingTime: true
faq:
    - question: "Kardiyak BT ağrılı mı?"
      answer: "Hayır, sadece IV kontrast enjeksiyonu yapılır. Ağrısız bir işlemdir."
    - question: "Kardiyak BT ne kadar sürer?"
      answer: "Toplam işlem süresi 15-30 dakikadır."
    - question: "Kardiyak BT radyasyon içerir mi?"
      answer: "Evet, düşük dozda radyasyon içerir. Modern cihazlarla doz minimize edilmiştir."
    - question: "Kardiyak BT anjiyografinin yerini alabilir mi?"
      answer: "Düşük-orta riskli hastalarda evet. Yüksek riskli hastalarda invaziv anjiyografi gerekebilir."
    - question: "Kardiyak BT sonucu ne zaman çıkar?"
      answer: "Sonuçlar genellikle aynı gün veya ertesi gün raporlanır."
---

{{< alert type="info" >}}
Bu içerik, Türkiye Cumhuriyeti Sağlık Bakanlığı mevzuatına ve tıbbi etik kurallara uygun olarak, toplum sağlığını korumak ve bilgilendirmek amacıyla hazırlanmıştır. Herhangi bir tanı, tedavi garantisi veya yönlendirme içermez. En doğru bilgi için yetkili bir sağlık kuruluşuna başvurunuz.
{{< /alert >}}


Kardiyak BT (Bilgisayarlı Tomografi), kalp ve koroner arterlerin non-invaziv olarak görüntülenmesidir. Koroner arter hastalığının taranmasında ve değerlendirilmesinde önemli bir yere sahiptir.

## Kardiyak BT Nedir?

Kardiyak BT, X-ışınları kullanarak kalbin ve koroner arterlerin detaylı kesitsel görüntülerini oluşturur. Kontrast madde ile birlikte kullanıldığında koroner arterlerin iç yapısı görüntülenebilir.

**Kardiyak BT Türleri:**

-   Koroner BT Anjiyografi (KBTA)
-   Kalsiyum Skorlama
-   Kardiyak BT (yapısal değerlendirme)

## Koroner BT Anjiyografi Endikasyonları

### Düşük-Orta Riskli Hastalar

-   Atipik göğüs ağrısı
-   Belirsiz stres testi
-   Düşük-orta koroner risk
-   Acil serviste göğüs ağrısı değerlendirmesi

### Anatomik Değerlendirme

-   Koroner arter anomalileri
-   Bypass greft değerlendirmesi
-   Stent açıklığı kontrolü
-   Pulmoner ven anatomisi

### Kalsiyum Skorlama

-   Kardiyovasküler risk değerlendirmesi
-   Asemptomatik yüksek riskli bireyler
-   Tedavi kararı desteği

## Kardiyak BT Öncesi Hazırlık

### Genel Hazırlık

-   4 saat açlık
-   Kafein kısıtlaması (24 saat)
-   Kalp hızı kontrolü

### İlaç Hazırlığı

-   Beta bloker (kalp hızını düşürmek için)
-   Nitrogliserin (koroner dilatasyon)
-   Kontrast alerjisi önlemi (gerekirse)

## Kardiyak BT Nasıl Yapılır?

### İşlem Adımları

1. **EKG bağlantısı**: Kalp ritmi senkronizasyonu
2. **IV erişim**: Kontrast enjeksiyonu için
3. **Kalp hızı kontrolü**: Gerekirse beta bloker
4. **Nitrogliserin**: Koroner dilatasyon
5. **Kontrast enjeksiyonu**: Otomatik enjektör
6. **Görüntüleme**: Nefes tutma ile
7. **Görüntü işleme**: 3D rekonstrüksiyon

### İşlem Süresi

Toplam süre: 15-30 dakika

## Kardiyak BT Sonuçları

### Kalsiyum Skoru

| Skor    | Risk Kategorisi |
| ------- | --------------- |
| 0       | Çok düşük risk  |
| 1-100   | Düşük risk      |
| 101-400 | Orta risk       |
| >400    | Yüksek risk     |

### Koroner Darlık Değerlendirmesi

-   Normal: Darlık yok
-   Minimal: <%25
-   Hafif: %25-49
-   Orta: %50-69
-   Ciddi: %70-99
-   Tıkalı: %100

## Kardiyak BT Avantajları ve Sınırlamaları

### Avantajlar

-   Non-invaziv
-   Hızlı sonuç
-   Yüksek negatif prediktif değer
-   Anatomik detay

### Sınırlamalar

-   Radyasyon maruziyeti
-   Kontrast madde gereksinimi
-   Yüksek kalp hızında görüntü kalitesi düşer
-   Ciddi kalsifikasyonda değerlendirme zorluğu

## Sık Sorulan Sorular

{{< faq-list >}}

## Randevu ve İletişim

Kardiyak BT için randevu almak isterseniz:

{{< contact-buttons >}}

📍 **Avrasya Hospital** - Beştelsiz Mah. 101. Sokak No:107, Zeytinburnu, İstanbul

📞 **Telefon:** +90 212 665 50 50 (Dahili: 4012)

---

_Bu içerik Doç. Dr. Habib Çil tarafından hazırlanmış olup, genel bilgilendirme amaçlıdır. Kesin tanı ve tedavi için mutlaka bir kardiyoloji uzmanına başvurunuz._

## İlgili Tanısal Yöntemler

Kardiyak BT ile bağlantılı diğer kardiyoloji testleri:

-   [Koroner Anjiyografi](/tr/hizmetler/kardiyak-anjiyografi/) - İnvaziv damar görüntüleme
-   [Miyokard Perfüzyon](/tr/hizmetler/miyokard-perfuzyon/) - Sintigrafi
-   [Ekokardiyografi](/tr/hizmetler/ekokardiyografi/) - Kalp ultrasonografisi
-   [Efor Testi](/tr/hizmetler/efor-testi/) - Egzersiz stres testi
-   [Koroner Arter Hastalığı](/tr/hizmetler/koroner-arter-hastaligi/) - Koroner arter hastalığı
