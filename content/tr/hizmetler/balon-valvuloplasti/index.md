---
title: "Balon Valvüloplasti - Kapak Balonla Genişletme"
description: "İstanbul'da balon valvüloplasti. Girişimsel kardiyoloji uzmanı Doç. Dr. Habib Çil ile mitral ve aort kapak genişletme. Kateter ile kapak tedavisi. Avrasya Hospital."
date: 2024-01-15
lastmod: 2024-12-04
draft: false
service_type: "MedicalProcedure"
medical_specialty: "Cardiology"
procedure_type: "TherapeuticProcedure"
body_location: "Heart"
translationKey: "balloon-valvuloplasty"
showBreadcrumbs: true
showTableOfContents: true
showReadingTime: true
faq:
    - question: "Balon valvüloplasti ağrılı mı?"
      answer: "Lokal veya genel anestezi altında yapıldığı için ağrı hissedilmez."
    - question: "Balon valvüloplasti sonrası iyileşme süresi ne kadar?"
      answer: "Genellikle 1-2 gün hastanede kalış yeterlidir. 1 hafta içinde normal aktivitelere dönülebilir."
    - question: "Balon valvüloplasti kalıcı mı?"
      answer: "Mitral darlıkta uzun süreli etki sağlar. Aort darlığında geçici etkilidir."
    - question: "Balon valvüloplasti riskleri nelerdir?"
      answer: "Ciddi mitral yetersizliği, tamponad ve embolizm nadir görülen komplikasyonlardır."
    - question: "Balon valvüloplasti için uygun muyum?"
      answer: "Kapak anatomisi ve eşlik eden durumlar değerlendirilerek karar verilir."
---

Balon valvüloplasti, daralmış kalp kapağının balon kateter kullanılarak genişletilmesi işlemidir. Özellikle mitral ve pulmoner kapak darlıklarında etkili bir tedavi yöntemidir. Girişimsel kardiyoloji uzmanı Doç. Dr. Habib Çil, balon valvüloplasti işleminde geniş deneyime sahiptir.

## Balon Valvüloplasti Nedir?

Balon valvüloplasti, kateter ucundaki balonun şişirilerek daralmış kapağın açılmasıdır. Açık kalp cerrahisine alternatif minimal invaziv bir yöntemdir.

**Balon Valvüloplasti Türleri:**

-   Mitral balon valvüloplasti (PMBV)
-   Pulmoner balon valvüloplasti
-   Aort balon valvüloplasti
-   Triküspit balon valvüloplasti

## Mitral Balon Valvüloplasti (PMBV)

### Endikasyonlar

-   Semptomatik mitral darlığı
-   Uygun kapak anatomisi
-   Ciddi mitral yetersizliği olmaması
-   Sol atriyumda pıhtı olmaması

### Wilkins Skoru

Kapak uygunluğunu değerlendirmek için kullanılır:

| Parametre     | 1       | 2     | 3        | 4         |
| ------------- | ------- | ----- | -------- | --------- |
| Mobilite      | Yüksek  | Orta  | Düşük    | Çok düşük |
| Kalınlaşma    | Minimal | Orta  | Belirgin | Ciddi     |
| Kalsifikasyon | Yok     | Hafif | Orta     | Ciddi     |
| Subvalvüler   | Normal  | Hafif | Orta     | Ciddi     |

Toplam skor ≤8: İyi aday

### İşlem Tekniği

1. Femoral ven erişimi
2. Transseptal ponksiyon
3. Balon kateteri sol atriyuma geçirme
4. Mitral kapaktan geçiş
5. Balon şişirme
6. Sonuç değerlendirmesi

## Pulmoner Balon Valvüloplasti

### Endikasyonlar

-   Konjenital pulmoner darlık
-   Gradiyent >40 mmHg
-   Semptomatik hastalar

### Teknik

-   Femoral ven erişimi
-   Sağ ventrikül üzerinden
-   Pulmoner kapakta balon şişirme

## Aort Balon Valvüloplasti

### Endikasyonlar

-   TAVI öncesi köprü tedavisi
-   Hemodinamik instabilite
-   Cerrahi için uygun olmayan hastalar

### Sınırlamalar

-   Geçici etki
-   Restenoz riski yüksek
-   Palyatif tedavi

## Balon Valvüloplasti Öncesi Hazırlık

### Değerlendirme

-   Transtorasik ekokardiyografi
-   Transözofageal ekokardiyografi
-   Sol atriyal pıhtı taraması
-   Mitral yetersizliği değerlendirmesi

### Hasta Hazırlığı

-   6-8 saat açlık
-   Antikoagülasyon düzenlemesi
-   Laboratuvar testleri
-   Anestezi değerlendirmesi

## Balon Valvüloplasti Nasıl Yapılır?

### İşlem Adımları

1. **Anestezi**: Lokal veya genel
2. **Vasküler erişim**: Femoral ven
3. **Transseptal ponksiyon**: Sol atriyuma geçiş
4. **Balon yerleştirme**: Kapak düzeyinde
5. **Balon şişirme**: Kademeli
6. **Sonuç kontrolü**: Gradiyent ölçümü
7. **Hemostaz**: Erişim bölgesi kapatma

### İşlem Süresi

Toplam süre: 1-2 saat

## Balon Valvüloplasti Sonuçları

### Başarı Kriterleri

-   Kapak alanı artışı
-   Gradiyent düşüşü
-   Semptom iyileşmesi
-   Ciddi komplikasyon olmaması

### Komplikasyonlar

-   Ciddi mitral yetersizliği
-   Kardiyak tamponad
-   Embolizm
-   Vasküler komplikasyonlar

## Sık Sorulan Sorular

{{< faq-list >}}

## Randevu ve İletişim

Balon valvüloplasti için randevu almak isterseniz:

{{< button href="https://wa.me/905339454639" target="_blank" >}}
WhatsApp ile Randevu Al
{{< /button >}}

📍 **Avrasya Hospital** - Beştelsiz Mah. 101. Sokak No:107, Zeytinburnu, İstanbul

📞 **Telefon:** +90 212 665 50 50 (Dahili: 4012)

---

_Bu içerik Doç. Dr. Habib Çil tarafından hazırlanmış olup, genel bilgilendirme amaçlıdır. Kesin tanı ve tedavi için mutlaka bir kardiyoloji uzmanına başvurunuz._

## İlgili Kapak Tedavileri

Balon valvüloplasti ile bağlantılı diğer yapısal kalp tedavileri:

-   [TAVI](/tr/hizmetler/tavi/) - Transkateter aort kapak implantasyonu
-   [Aort Kapak Hastalığı](/tr/hizmetler/aort-kapak-hastaligi/) - Aort kapak hastalıkları hakkında
-   [Mitral Kapak](/tr/hizmetler/mitral-kapak/) - Mitral kapak hastalıkları
-   [MitraClip](/tr/hizmetler/mitraclip/) - Mitral kapak tamiri
-   [Ekokardiyografi](/tr/hizmetler/ekokardiyografi/) - Kapak değerlendirmesi
