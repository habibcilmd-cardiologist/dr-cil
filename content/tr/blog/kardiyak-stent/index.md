---
title: 'Kardiyak Stent: İlaç Salınımlı Stent İmplantasyonu Rehberi'
description: Kardiyak stent nedir? İlaç salınımlı stent (DES), çıplak metal stent (BMS), stent implantasyonu işlemi, stent sonrası tedavi ve bakım hakkında kapsamlı bilgi.
date: 2024-12-10
lastmod: 2024-12-10
translationKey: cardiac-stent
categories:
- Kardiyoloji
- Girişimsel Kardiyoloji
tags:
- Stent
- Koroner Stent
author: Doç. Dr. Habib ÇİL
showTableOfContents: true
featured: featured.svg
faq:
- question: Stent kalıcı mı?
  answer: Evet, metal stentler kalıcıdır. Biyoemilebilir stentler zamanla erir.
- question: Stent sonrası ilaç kullanımı ne kadar sürer?
  answer: İkili antiplatelet tedavi genellikle 6-12 ay sürer. Aspirin ömür boyu devam eder.
- question: Stent sonrası MR çekilebilir mi?
  answer: Evet, modern stentler MR uyumludur.
- question: Stent sonrası spor yapabilir miyim?
  answer: Evet, iyileşme sonrası düzenli egzersiz önerilir.
- question: Stent tıkanabilir mi?
  answer: Nadir görülür. İlaç uyumu ve yaşam tarzı değişikliği önemlidir.
---

{{< alert type="info" >}}
Bu içerik, Türkiye Cumhuriyeti Sağlık Bakanlığı mevzuatına ve tıbbi etik kurallara uygun olarak, toplum sağlığını korumak ve bilgilendirmek amacıyla hazırlanmıştır. Herhangi bir tanı, tedavi garantisi veya yönlendirme içermez. En doğru bilgi için yetkili bir sağlık kuruluşuna başvurunuz.
{{< /alert >}}

{{< lead >}}
Kardiyak stent, koroner arterlerdeki darlıkların açık tutulması için yerleştirilen metal kafes yapısıdır. Koroner arter hastalığının tedavisinde yaygın olarak kullanılır. Bu kapsamlı rehberde stent türleri, implantasyon işlemi ve stent sonrası bakım detaylı olarak ele alınmaktadır.
{{< /lead >}}

## Kardiyak Stent Nedir?

Kardiyak stent, balon anjiyoplasti sonrası damarın açık kalmasını sağlayan silindirik metal kafestir. Damar duvarına yapışarak darlığın tekrarlamasını önler.

**Stent Türleri:**

-   Çıplak metal stent (BMS)
-   İlaç kaplı stent (DES)
-   Biyoemilebilir stent

## Kardiyak Stent Endikasyonları

### Akut Koroner Sendrom

-   ST yükselmeli miyokard enfarktüsü
-   ST yükselmesiz miyokard enfarktüsü
-   Kararsız angina

### Stabil Koroner Arter Hastalığı

-   Ciddi darlık (>%70)
-   Semptomatik hastalar
-   Pozitif stres testi
-   Medikal tedaviye yanıtsız

## Stent Türleri

### Çıplak Metal Stent (BMS)

-   İlk nesil stentler
-   Daha yüksek restenoz oranı
-   Kısa süreli antiplatelet tedavi

### İlaç Kaplı Stent (DES)

-   Antiproliferatif ilaç kaplı
-   Düşük restenoz oranı
-   Uzun süreli antiplatelet tedavi
-   Günümüzde standart

### Biyoemilebilir Stent

-   Zamanla eriyen yapı
-   Damar fonksiyonu korunur
-   Seçilmiş hastalarda

## Stent İmplantasyonu Nasıl Yapılır?

### Hazırlık

-   Anjiyografi ile değerlendirme
-   Antiplatelet yükleme
-   Hidrasyon

### İşlem Adımları

1. **Damar erişimi**: Femoral veya radyal
2. **Kılavuz kateter**: Koroner ostiumuna
3. **Kılavuz tel**: Lezyondan geçiş
4. **Predilatasyon**: Balon ile
5. **Stent yerleştirme**: Lezyon üzerine
6. **Stent açılması**: Balon şişirme
7. **Postdilatasyon**: Gerekirse
8. **Kontrol anjiyografi**: Sonuç değerlendirmesi

### İşlem Süresi

Toplam süre: 30-90 dakika

## Stent Sonrası Tedavi

### Antiplatelet Tedavi

**İkili Antiplatelet Tedavi (DAPT):**

-   Aspirin + P2Y12 inhibitörü
-   Süre: 6-12 ay (DES)
-   Süre: 1-3 ay (BMS)

### Diğer İlaçlar

-   Statin
-   Beta bloker
-   ACE inhibitörü
-   Nitrat (gerekirse)

## Stent Komplikasyonları

### Erken Komplikasyonlar

-   Stent trombozu
-   Diseksiyon
-   Perforasyon

### Geç Komplikasyonlar

-   In-stent restenoz
-   Geç stent trombozu
-   Neoateroskleroz

## Sık Sorulan Sorular

{{< faq-list >}}

## Randevu ve İletişim

Kardiyak stent değerlendirmesi için randevu almak isterseniz:

{{< button href="https://wa.me/905339454639" target="_blank" >}}
WhatsApp ile Randevu Al
{{< /button >}}

📍 **Avrasya Hospital** - Beştelsiz Mah. 101. Sokak No:107, Zeytinburnu, İstanbul

📞 **Telefon:** +90 212 665 50 50 (Dahili: 4012)

---

_Bu içerik Doç. Dr. Habib Çil tarafından hazırlanmış olup, genel bilgilendirme amaçlıdır. Kesin tanı ve tedavi için mutlaka bir kardiyoloji uzmanına başvurunuz._
