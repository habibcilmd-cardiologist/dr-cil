---
title: 'Kardiyak Kateterizasyon: Kalp Kateteri İşlemi Rehberi'
description: Kardiyak kateterizasyon nedir? Kalp odacıkları ve damarlarında tanısal ve tedavi amaçlı kateter işlemi, sol ve sağ kalp kateterizasyonu hakkında kapsamlı bilgi.
date: 2024-12-10
lastmod: 2024-12-10
translationKey: cardiac-catheterization
categories:
- Kardiyoloji
- Girişimsel Kardiyoloji
tags:
- Kateterizasyon
- Kalp Kateteri
author: Doç. Dr. Habib ÇİL
showTableOfContents: true
featured: featured.svg
faq:
- question: Kardiyak kateterizasyon ağrılı mı?
  answer: Lokal anestezi ile yapılır ve genellikle ağrı hissedilmez.
- question: Kateterizasyon ne kadar sürer?
  answer: Tanısal işlem 30-60 dakika, girişimsel işlemler daha uzun sürebilir.
- question: Kateterizasyon sonrası ne zaman taburcu olurum?
  answer: Genellikle aynı gün veya ertesi gün taburcu olunabilir.
- question: Kateterizasyon riskleri nelerdir?
  answer: Ciddi komplikasyonlar nadir görülür. Kanama ve enfeksiyon riski düşüktür.
- question: Kateterizasyon sırasında uyanık mıyım?
  answer: Evet, lokal anestezi ile yapılır ve işlem boyunca uyanıksınız.
---

{{< alert type="info" >}}
Bu içerik, Türkiye Cumhuriyeti Sağlık Bakanlığı mevzuatına ve tıbbi etik kurallara uygun olarak, toplum sağlığını korumak ve bilgilendirmek amacıyla hazırlanmıştır. Herhangi bir tanı, tedavi garantisi veya yönlendirme içermez. En doğru bilgi için yetkili bir sağlık kuruluşuna başvurunuz.
{{< /alert >}}

{{< lead >}}
Kardiyak kateterizasyon, kalp odacıkları ve damarlarına kateter yerleştirilerek yapılan tanısal ve tedavi amaçlı işlemdir. Bu kapsamlı rehberde kardiyak kateterizasyon türleri, endikasyonları ve işlem sürecini detaylı olarak ele alınmaktadır.
{{< /lead >}}

## Kardiyak Kateterizasyon Nedir?

Kardiyak kateterizasyon, ince bir tüpün (kateter) damar yoluyla kalbe ilerletilmesidir. Basınç ölçümü, görüntüleme ve tedavi işlemleri yapılabilir.

**Kardiyak Kateterizasyon Türleri:**

-   Sol kalp kateterizasyonu
-   Sağ kalp kateterizasyonu
-   Kombine kateterizasyon

## Kardiyak Kateterizasyon Endikasyonları

### Tanısal Amaçlar

-   Koroner arter hastalığı değerlendirmesi
-   Kapak hastalığı değerlendirmesi
-   Kalp yetmezliği değerlendirmesi
-   Konjenital kalp hastalığı

### Tedavi Amaçları

-   Koroner anjiyoplasti ve stent
-   Balon valvüloplasti
-   Septal defekt kapatma
-   TAVI

## Sol Kalp Kateterizasyonu

### Endikasyonlar

-   Koroner arter hastalığı
-   Aort kapak hastalığı
-   Mitral kapak hastalığı
-   Sol ventrikül fonksiyonu

### Ölçümler

-   Sol ventrikül basıncı
-   Aort basıncı
-   Koroner anjiyografi
-   Ventrikulografi

## Sağ Kalp Kateterizasyonu

### Endikasyonlar

-   Pulmoner hipertansiyon
-   Kalp yetmezliği
-   Kapak hastalığı
-   Şant değerlendirmesi

### Ölçümler

-   Sağ atrium basıncı
-   Sağ ventrikül basıncı
-   Pulmoner arter basıncı
-   Pulmoner kapiller wedge basıncı
-   Kardiyak output

## Kardiyak Kateterizasyon Nasıl Yapılır?

### Hazırlık

-   6-8 saat açlık
-   Böbrek fonksiyon testleri
-   Kan sulandırıcı düzenlemesi

### İşlem Adımları

1. **Lokal anestezi**: Giriş bölgesine
2. **Damar erişimi**: Femoral veya radyal
3. **Kateter ilerletme**: Kalbe doğru
4. **Basınç ölçümü**: Odacıklarda
5. **Görüntüleme**: Kontrast ile
6. **Tedavi**: Gerekirse
7. **Kateter çıkarma**: İşlem sonu

### İşlem Süresi

Toplam süre: 30-60 dakika

## Kardiyak Kateterizasyon Sonuçları

### Normal Değerler

| Parametre      | Normal Değer       |
| -------------- | ------------------ |
| Sol ventrikül  | 100-140/5-12 mmHg  |
| Aort           | 100-140/60-90 mmHg |
| Sağ ventrikül  | 15-30/0-8 mmHg     |
| Pulmoner arter | 15-30/4-12 mmHg    |

## Komplikasyonlar

-   Kanama (nadir)
-   Enfeksiyon (nadir)
-   Aritmi
-   Kontrast reaksiyonu
-   Damar hasarı

## Sık Sorulan Sorular

{{< faq-list >}}

## Randevu ve İletişim

Kardiyak kateterizasyon için randevu almak isterseniz:

{{< button href="https://wa.me/905339454639" target="_blank" >}}
WhatsApp ile Randevu Al
{{< /button >}}

📍 **Avrasya Hospital** - Beştelsiz Mah. 101. Sokak No:107, Zeytinburnu, İstanbul

📞 **Telefon:** +90 212 665 50 50 (Dahili: 4012)

---

_Bu içerik Doç. Dr. Habib Çil tarafından hazırlanmış olup, genel bilgilendirme amaçlıdır. Kesin tanı ve tedavi için mutlaka bir kardiyoloji uzmanına başvurunuz._
